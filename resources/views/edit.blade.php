    ...
    <body>
        <div class="productCreate-container">
            ...
            <form action="{{route('update',['stock' => $stockEdit])}}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="title-wrap">商品タイトル : <input type="text" name="name" placeholder="商品タイトル" value="{{old('name',$stockEdit->name)}}"></div>
                <div class="img-wrap">商品画像 : <input type="file" name="imgpath" accept="image/jpg,image/png"></div>
                <div class="detail-wrap">商品説明 : <textarea type="text" name="detail" cols="30" rows="10" placeholder="商品説明">{{old('detail',$stockEdit->detail)}}</textarea></div>
                <div class="fee-wrap">値段 : <input type="text" name="fee" placeholder="10000" value="{{old('fee',$stockEdit->fee)}}">円</div>
